...
`check` проверяет совпадение версий платформ, линтер, строгий TypeScript и весь набор Vitest в одном thread worker. Такой режим уменьшает расход памяти и конкуренцию тестов за ресурсы. Для наблюдения за тестами используйте `npm run test:watch`.

E2E собирает production web-приложение и запускает отдельный preview-сервер на `127.0.0.1:4173`. Playwright управляет его запуском и остановкой и отказывается использовать занятый порт. На Linux при отсутствии системных библиотек выполните `npx playwright install --with-deps chromium`. Диагностика неуспешных сценариев попадает в `test-results/` и, в CI, `playwright-report/`.
