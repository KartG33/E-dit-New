import { useAndroidViewport } from './hooks/useAndroidViewport';
...
import { insertSunoTag, type SunoTagOccurrence } from '@core/commands/suno';
...
import { COMMAND_REGISTRY, type CommandId } from '@core/commands/registry';
...
const App = () => {
  ...
  const runAction = (id: string) => {
    ...
    switch (id) {
      ...
      case 'open.presets': openPanel({ type: 'settings', view: 'presets' }); return;
  ...
  return <ActionContext.Provider value={{ title: actionTitle, run: runAction }}><div className="app-shell">
    ...
    <CommandPanel
      ...
      onOpenHistory={() => openPanel({ type: 'history' })}
      onOpenSettings={() => openPanel({ type: 'settings' })}
    />
