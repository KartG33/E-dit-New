import { useRef, useState, useEffect, useLayoutEffect, useImperativeHandle, type Ref } from 'react';
...
import { useEditorKeyboardOverlay } from '../../hooks/useEditorKeyboardOverlay';
import { useSymbolAnalyzer } from '../../hooks/useSymbolAnalyzer';
import { useGlobalHotkeys } from '../../hooks/useGlobalHotkeys';
import type { EditorState } from '../../hooks/useEditor';
import { removeTokenFromText } from '@core/analyzer';
...
export const Editor = ({
  ...
  const [symbolsExpanded, setSymbolsExpanded] = useState(() => {
    try { return localStorage.getItem(`edit.symbols.${id}`) === 'expanded'; }
    catch { return false; }
  });
  const toggleSymbols = () => {
    const next = !symbolsExpanded;
    setSymbolsExpanded(next);
    try { localStorage.setItem(`edit.symbols.${id}`, next ? 'expanded' : 'collapsed'); }
    catch { notify('Could not remember the symbols panel state', true); }
  };
  const textareaRef = useRef<HTMLTextAreaElement>(null);
  const cardRef = useRef<HTMLDivElement>(null);
  const footerRef = useRef<HTMLDivElement>(null);
  useEditorKeyboardOverlay(cardRef, textareaRef, footerRef, symbolsExpanded, isActive, value);
  ...
  return (
    <div ref={cardRef} className={`editor-card ${isActive ? 'is-active' : ''}`}>
      <div className="editor-header">
        ...
        <button type="button" className="symbols-toggle" aria-label={`Symbols ${id} editor`}
          aria-pressed={symbolsExpanded} aria-expanded={symbolsExpanded} aria-controls={`symbols-${id}`} onPointerDown={preserveEditorFocus} onClick={toggleSymbols}>
          S
        </button>
      ...
      {(
        <div ref={footerRef} id={`symbols-${id}`} className={`editor-footer ${symbolsExpanded ? '' : 'symbols-collapsed'}`}>
          <div className="token-list">
            {stats.tokens.length === 0 && <span className="symbols-empty">No symbols</span>}
            {stats.tokens.map(t => (
              <button
                type="button"
                key={t.token}
