...
import { ArrowLeft, ChevronRight, HardDrive, Keyboard, SlidersHorizontal, X } from 'lucide-react';
...
type SettingsView = 'home' | 'keys' | 'data' | 'presets';
...
export const SettingsModal = ({ presets, onClose, initialView = 'home' }: SettingsModalProps) => {
  ...
  const { dialogRef, backdropProps } = useDialog(onClose, view !== 'presets');
  useEffect(() => {
    const back = (event: Event) => {
      if (event.defaultPrevented || view === 'home' || view === 'presets') return;
  ...
  useEffect(() => {
    const closeOnEscape = (event: KeyboardEvent) => {
      if (view !== 'presets' && event.key === 'Escape' && !event.defaultPrevented) onClose();
    };
    document.addEventListener('keydown', closeOnEscape);
    return () => document.removeEventListener('keydown', closeOnEscape);
  }, [onClose, view]);

  if (view === 'presets') return <PresetManager onClose={() => setView('home')} onBack={() => setView('home')} />;
        ...
        <div className="settings-modal-content">
          {view === 'home' && (
            <div className="settings-section-list">
              <button type="button" className="settings-section-card settings-presets-section" onClick={() => setView('presets')}>
                <span className="settings-section-icon"><SlidersHorizontal size={20} /></span>
                <span className="settings-section-copy"><strong>Presets</strong><small>Manage presets</small></span>
