...
export function ShortcutRecorder({ value, onChange, validate = validatePresetShortcut, onError, label }: Props) {
  ...
  useEffect(() => {
    if (!recording) return;
    const back = (event: Event) => { event.preventDefault(); setRecording(false); };
    ...
    return () => { window.removeEventListener('keydown', capture, true); window.removeEventListener(ANDROID_BACK_REQUEST_EVENT, back); };
