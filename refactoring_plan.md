...
* **Неиспользуемая сущность `Note` в базе данных:**
  * В [src/lib/db/index.ts](file:///d:/Documents/Antigravity%20Projects/E-dit%20New/src/lib/db/index.ts#L4-L11) объявлен интерфейс `Note` и таблица `notes` (`EditDatabase.notes`), однако вкладка Notes вырезана из UI и тестов.
...
```mermaid
graph TD
    E1[Этап 1: Чистка неиспользуемого кода и нормализация именования] --> E2[Этап 2: Исправление версионирования и валидации данных]
...
* **Что исправляется:** Удаляется неиспользуемый файл `src/App.css`, очищается неиспользуемый интерфейс `Note` / таблица `notes` (с сохранением обратной совместимости Dexie schema v1-v3), приводятся к единообразию имена функций в `src/lib/commands/`.
